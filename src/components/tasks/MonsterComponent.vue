...
<template>
    <div  class="w-auto h-max shadow-md bg-white rounded-2xl py-5 px-10 ">
        ...
        <div class="flex justify-between">
            <div>
                <h1 class="text-green-700 font-bold">{{ monster.title }}</h1>
                <p class="text-black">{{ monster.description }}</p>
            </div>
            <div>
                <h1 class="text-black">
                    <span class="font-black text-red-500"> {{ player.calculateXP(monster) }} </span>xp
                </h1>
                <h1 class="text-black">
                    <span class="font-black text-blue-700"> {{ player.calculateCompletionTime(monster) }}
                    </span>sec
                </h1>
            </div>
        </div>
        <h1 class="text-black"><span class="font-black">Due date</span>
            <span v-if="monster.dueDate != undefined">
                {{ getDayOfTheWeek(monster.dueDate.getDay()) }}, {{ monster.dueDate.toLocaleString() }}
            </span>
        </h1>
        <div class="flex flex-row justify-end pt-5">
            <div class="w-4">
            </div>
            <button id="show-modal" @click="selectTask(monster, index)"
                class="w-max h-max px-4 py-2 rounded-md bg-green-500 hover:bg-green-400 active:bg-green-600">
                <h1 class="text-black font-bold select-none">Fight</h1>
            </button>
        </div>
    </div>
    <div class="h-2" />
